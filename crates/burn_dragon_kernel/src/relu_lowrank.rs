...
fn fused_relu_lowrank_autodiff_cuda(
    ...
    let output = relu_lowrank_runtime::<CudaRuntime>(
        input_inner.clone(),
        weight_inner.clone(),
        shape,
        meta,
...
/// Executes the reference forward path while still routing backward through the custom WGPU
/// low-rank projection autodiff implementation when supported.
fn try_direct_path_runtime<B: BackendTrait, R: CubeRuntime + 'static>(
    input: &BurnTensor<B, 4>,
    weight: &BurnTensor<B, 4>,
    sparse_mask: Option<&BurnTensor<B, 4>>,
    meta: &BurnTensor<B, 1>,
...
fn try_direct_path_autodiff_cube_runtime<B: BackendTrait, R: CubeRuntime + 'static>(
    input: &BurnTensor<B, 4>,
    weight: &BurnTensor<B, 4>,
    threshold: f32,
    sparse_mask: Option<&BurnTensor<B, 4>>,
...
fn relu_lowrank_runtime<R: CubeRuntime + 'static>(
    input: CubeTensor<R>,
    weight: CubeTensor<R>,
    shape: LowrankProjectionShape,
    meta: CubeTensor<R>,
...
fn relu_lowrank_wgsl_runtime<R: CubeRuntime>(
    input: CubeTensor<R>,
    weight: CubeTensor<R>,
    shape: LowrankProjectionShape,
    meta: CubeTensor<R>,
...
mod tests {
    ...
    fn fused_relu_lowrank_matches_reference_single_stream_query_weight_gradients_on_wgpu_autodiff()
    {
        ...
        let input =
            Tensor::<AutodiffBackendImpl, 4>::random([2, 1, 7, 16], Distribution::Default, &device)
                .require_grad();
        let weight = Tensor::<AutodiffBackendImpl, 4>::random(
            [1, 4, 16, 12],
            Distribution::Default,
            &device,
        )
        .require_grad();
        let mask = Tensor::<AutodiffBackendImpl, 1>::from_floats([1.0; 12], &device)
            .reshape([1, 1, 1, 12]);
    ...
    fn fused_relu_lowrank_matches_reference_single_stream_query_weight_gradients_on_wgpu_autodiff_long_sequence()
     {
        ...
        let input = Tensor::<AutodiffBackendImpl, 4>::random(
            [1, 1, 401, 8],
            Distribution::Default,
            &device,
        )
        ...
        .expect("fused autodiff");
        let reference =
            lowrank_projection_reference_forward(input.clone(), weight.clone(), 0.05, None);
    ...
    fn fused_relu_lowrank_kernel_tiled_matches_reference_head_aligned_query_weight_gradients_on_wgpu_fusion_autodiff_long_sequence()
     {
        ...
        let fused =
            try_fused_relu_lowrank_projection_wgpu_with_executor::<FusionAutodiffBackendImpl>(
                &input,
                &weight,
                0.05,
                None,
    ...
    fn fused_relu_lowrank_matches_reference_single_stream_on_cuda() {
        ...
        let weight =
            Tensor::<CudaBackend, 4>::random([1, 4, 32, 16], Distribution::Default, &device);
    ...
    fn fused_relu_lowrank_matches_reference_gradients_on_cuda_autodiff() {
        ...
        let input = Tensor::<CudaAutodiffBackendImpl, 4>::random(
            [1, 1, 5, 16],
            Distribution::Default,
            &device,
        )
        .require_grad();
        let weight = Tensor::<CudaAutodiffBackendImpl, 4>::random(
            [1, 2, 16, 8],
            Distribution::Default,
            &device,
        )
        .require_grad();
        let mask = Tensor::<CudaAutodiffBackendImpl, 1>::from_floats([1.0; 8], &device)
            .reshape([1, 1, 1, 8]);
