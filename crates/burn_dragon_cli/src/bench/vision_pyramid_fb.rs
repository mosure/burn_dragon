...
use burn_dragon_wgpu::api::spatial::{
    ...
    structured_pyramid_profile_snapshot, try_fused_structured_pyramid_rho_step_wgpu_with_plan,
};
...
#[derive(Clone, Debug)]
pub struct VisionPyramidForwardBackwardBenchConfig {
    pub warmup: usize,
...
pub struct VisionPyramidForwardBackwardBenchCase {
    name: &'static str,
    batch: usize,
    patch_height: usize,
    patch_width: usize,
...
#[derive(Clone, Copy, Serialize)]
pub struct VisionPyramidForwardBackwardBenchErrorMetrics {
    max_abs: f32,
    mean_abs: f32,
}

#[derive(Clone, Serialize)]
pub struct VisionPyramidForwardBackwardBenchMeasurementResult {
    mode: &'static str,
    steps: usize,
...
#[derive(Clone, Serialize)]
pub struct VisionPyramidForwardBackwardBenchCaseResult {
    pub case: VisionPyramidForwardBackwardBenchCase,
    pub problem_shape: String,
    pub one_step: VisionPyramidForwardBackwardBenchMeasurementResult,
    pub resident_rollout: VisionPyramidForwardBackwardBenchMeasurementResult,
    pub loss_abs_diff: f32,
    pub patch_context_error: VisionPyramidForwardBackwardBenchErrorMetrics,
    pub coarse_context_error: VisionPyramidForwardBackwardBenchErrorMetrics,
    pub patch_rho_error: VisionPyramidForwardBackwardBenchErrorMetrics,
    pub coarse_rho_error: VisionPyramidForwardBackwardBenchErrorMetrics,
    pub hub_rho_error: VisionPyramidForwardBackwardBenchErrorMetrics,
}

#[derive(Clone, Serialize)]
pub struct VisionPyramidForwardBackwardBenchReport {
    ...
    pub repetitions: usize,
    pub steps: usize,
    pub cases: Vec<VisionPyramidForwardBackwardBenchCaseResult>,
}

const CASES: &[VisionPyramidForwardBackwardBenchCase] = &[
    VisionPyramidForwardBackwardBenchCase {
        name: "small",
        batch: 2,
        patch_height: 8,
        patch_width: 8,
...
pub fn init_vision_pyramid_forward_backward_bench_runtime(device: &Device) {
...
impl VisionPyramidForwardBackwardBenchReport {
    pub fn to_markdown(&self) -> String {
        format_markdown(self)
    }
}

pub fn run_vision_pyramid_forward_backward_bench(
    config: &VisionPyramidForwardBackwardBenchConfig,
) -> Result<VisionPyramidForwardBackwardBenchReport> {
    let device = Device::default();
    init_vision_pyramid_forward_backward_bench_runtime(&device);

    Ok(VisionPyramidForwardBackwardBenchReport {
        artifact: VisionArtifactHeader::new("vision_pyramid_fb_bench"),
...
fn run_case(
    case: VisionPyramidForwardBackwardBenchCase,
    device: &Device,
    config: &VisionPyramidForwardBackwardBenchConfig,
) -> VisionPyramidForwardBackwardBenchCaseResult {
    ...
    let hub_rho = Tensor::<TrainBackend, 4>::zeros(
        [case.batch, case.hub_count, case.rank, case.value_dim],
        device,
    );
    ...
    let (
        ...
        coarse_rho_error,
        hub_rho_error,
    ) = parity_snapshot(shape, input.clone(), &plan, config.steps.max(1), device);
    ...
    let mut fused_one_step = Vec::with_capacity(config.repetitions);
    let mut baseline_rollout = Vec::with_capacity(config.repetitions);
    let mut fused_rollout = Vec::with_capacity(config.repetitions);

    for _ in 0..config.repetitions {
        baseline_one_step.push(run_forward_backward(shape, input.clone(), None, 1, device));
        fused_one_step.push(run_forward_backward(
            shape,
            input.clone(),
            Some(&plan),
            1,
...
fn parity_snapshot(
    ...
    f32,
    VisionPyramidForwardBackwardBenchErrorMetrics,
    VisionPyramidForwardBackwardBenchErrorMetrics,
    VisionPyramidForwardBackwardBenchErrorMetrics,
    VisionPyramidForwardBackwardBenchErrorMetrics,
    VisionPyramidForwardBackwardBenchErrorMetrics,
    ...
    (
        loss_abs_diff,
        error_metrics(
            reference.patch_local_context.inner(),
            fused.patch_local_context.inner(),
        ),
        error_metrics(
        ...
        error_metrics(
            reference.next_patch_rho.inner(),
            fused.next_patch_rho.inner(),
        ),
        error_metrics(
            reference.next_coarse_rho.inner(),
            fused.next_coarse_rho.inner(),
        ),
...
fn error_metrics<const D: usize>(
    ...
    VisionPyramidForwardBackwardBenchErrorMetrics { max_abs, mean_abs }
}
...
fn summarize_measurements(
    ...
    fused: &[StepMetrics],
) -> VisionPyramidForwardBackwardBenchMeasurementResult {
...
fn format_markdown(report: &VisionPyramidForwardBackwardBenchReport) -> String {
    let mut out = String::new();
    push_vision_artifact_markdown_prelude(
        &mut out,
        "Vision Pyramid Forward+Backward Benchmark",
        &report.artifact,
    );
