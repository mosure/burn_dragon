...
pub(crate) fn init_momentum_teacher<B, M>(
    student: &M,
    config: &VisionMomentumTeacherConfig,
) -> Option<M>
where
    B: BackendTrait,
    M: Module<B> + Clone,
...
pub(crate) fn ema_update_module<B, M>(teacher: M, student: &M, decay: f32) -> M
where
    B: BackendTrait,
    M: Module<B> + Clone,
{
    ...
    impl<B: BackendTrait> ModuleMapper<B> for EmaBlendMapper {
...
pub(crate) fn sync_optional_teacher_from_student<B, M>(
    teacher: Option<M>,
    student: &M,
    enabled: bool,
    decay: f32,
    ...
    M: Module<B> + Clone,
{
    ...
    Some(match teacher {
        Some(teacher) => ema_update_module::<B, _>(teacher, student, decay),
        None => student.clone().no_grad(),
    })
}

pub(crate) fn restore_optional_teacher_from_student<B, M>(student: &M, enabled: bool) -> Option<M>
where
    B: BackendTrait,
    M: Module<B> + Clone,
{
