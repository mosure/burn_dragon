...
fn patchify_roundtrip_with_padding() {
    ...
    let patches = patchify(images.clone(), patch_size);
    let [patch_batch, tokens, patch_dim] = patches.shape().dims::<3>();
    let grid_h = (height + patch_size - 1) / patch_size;
    let grid_w = (width + patch_size - 1) / patch_size;
    assert_eq!(patch_batch, batch);
...
fn lejepa_recon_psnr_improves_on_toy_batch() {
    ...
    lejepa_config.global_views = 0;
    lejepa_config.local_views = 0;
    lejepa_config.artifact_every = 0;
    lejepa_config.artifact_max_images = 0;
    lejepa_config.artifact_max_views = 0;
