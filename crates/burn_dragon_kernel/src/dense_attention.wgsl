...
var<workgroup> partial_sums: array<f32, WORKGROUP_SIZE_X>;
var<workgroup> row_denom: f32;
...
fn main(
  ...
  var stride = WORKGROUP_SIZE_X / 2u;
  loop {
    if stride == 0u {
      break;
    }
  ...
  col = 0u;
  while col < time {
    acc += (row_scores[col] / row_denom) * value[idx_value(b, value_head, col, e, value_heads, time, value_dim)];
    col += 1u;
  }
