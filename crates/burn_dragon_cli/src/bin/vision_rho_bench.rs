...
fn main() {
    ...
    let json = serde_json::to_string_pretty(&report).expect("serialize vision rho report");
    println!("{markdown}");
...
fn run_case(case: BenchCase, device: &Device, args: &Args) -> CaseResult {
    ...
    let initial_rho =
        Tensor::<Backend, 5>::zeros([case.batch, case.heads, tokens, case.latent, case.embd], device);
    let decay_values = (0..case.heads)
        .map(|idx| 0.85_f32 + 0.1_f32 * (idx as f32 / case.heads.max(1) as f32))
        .collect::<Vec<_>>();
    let decay = Tensor::<Backend, 1>::from_data(TensorData::new(decay_values, [case.heads]), device);
    ...
    let rollout_tokens = (case.batch * tokens * args.steps.max(1)) as f64;
...
fn reference_rollout(
    ...
    let mut context = Tensor::<Backend, 4>::zeros(
        [query.shape().dims::<4>()[0], query.shape().dims::<4>()[1], grid.token_count(), value.shape().dims::<4>()[3]],
        &query.device(),
    );
...
fn compare_tensors<const D: usize>(lhs: Tensor<Backend, D>, rhs: Tensor<Backend, D>) -> ErrorMetrics {
    let lhs = lhs
        .into_data()
        .convert::<f32>()
        .into_vec::<f32>()
...
fn format_markdown(report: &Report) -> String {
    ...
    let _ = writeln!(&mut out, "# Vision Rho Benchmark");
