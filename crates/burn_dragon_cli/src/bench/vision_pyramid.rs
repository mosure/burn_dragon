...
#[derive(Clone, Debug)]
pub struct VisionPyramidBenchConfig {
    pub warmup: usize,
...
pub struct VisionPyramidBenchCase {
    name: &'static str,
    batch: usize,
    patch_height: usize,
    patch_width: usize,
...
#[derive(Clone, Copy, Serialize)]
pub struct VisionPyramidBenchErrorMetrics {
    max_abs: f32,
    mean_abs: f32,
}

#[derive(Clone, Serialize)]
pub struct VisionPyramidBenchMeasurementResult {
    mode: &'static str,
...
#[derive(Clone, Serialize)]
pub struct VisionPyramidBenchCaseResult {
    pub case: VisionPyramidBenchCase,
    pub problem_shape: String,
    pub forward: VisionPyramidBenchMeasurementResult,
    pub resident_rollout: VisionPyramidBenchMeasurementResult,
    pub patch_local_error: VisionPyramidBenchErrorMetrics,
    pub coarse_local_error: VisionPyramidBenchErrorMetrics,
    pub patch_rho_error: VisionPyramidBenchErrorMetrics,
    pub coarse_rho_error: VisionPyramidBenchErrorMetrics,
    pub hub_rho_error: VisionPyramidBenchErrorMetrics,
}

#[derive(Clone, Serialize)]
pub struct VisionPyramidBenchReport {
    ...
    pub repetitions: usize,
    pub steps: usize,
    pub cases: Vec<VisionPyramidBenchCaseResult>,
}

const CASES: &[VisionPyramidBenchCase] = &[
    VisionPyramidBenchCase {
        name: "small",
        batch: 2,
        patch_height: 8,
        patch_width: 8,
...
pub fn init_vision_pyramid_bench_runtime(device: &Device) {
...
impl VisionPyramidBenchReport {
    pub fn to_markdown(&self) -> String {
        format_markdown(self)
    }
}

pub fn run_vision_pyramid_bench(
    config: &VisionPyramidBenchConfig,
) -> Result<VisionPyramidBenchReport> {
    let device = Device::default();
    init_vision_pyramid_bench_runtime(&device);

    Ok(VisionPyramidBenchReport {
        artifact: VisionArtifactHeader::new("vision_pyramid_bench"),
...
fn run_case(
    case: VisionPyramidBenchCase,
    device: &Device,
    config: &VisionPyramidBenchConfig,
) -> VisionPyramidBenchCaseResult {
    ...
    let coarse_rho = Tensor::<Backend, 5>::zeros(
        [
            case.batch,
            case.rank,
            case.value_dim,
    ...
    let hub_rho = Tensor::<Backend, 4>::zeros(
...
fn error_metrics<const D: usize>(
...
) -> VisionPyramidBenchErrorMetrics {
    let diff = lhs
        .sub(rhs)
        .abs()
        .to_data()
    ...
    VisionPyramidBenchErrorMetrics { max_abs, mean_abs }
}
...
fn format_markdown(report: &VisionPyramidBenchReport) -> String {
    let mut out = String::new();
    push_vision_artifact_markdown_prelude(&mut out, report.benchmark, &report.artifact);
